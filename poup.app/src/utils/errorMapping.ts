...
const errorMessagesMap: Record<string, string> = {
  ...
  'A data de pagamento não pode ser no passado':
    'Escolha uma data futura para o próximo pagamento.',
  ...
  'Erro de validação dos dados':
    'Alguns campos estão preenchidos incorretamente. Verifique e tente novamente.',

  'Você não tem permissão para editar esta assinatura':
    'Você não pode editar assinaturas de outros usuários.',
  'Você não tem permissão para deletar esta assinatura':
    'Você não pode deletar assinaturas de outros usuários.',
...
export function handleApiError(error: any): AppError {
  if (error.response?.data) {
    ...
    if (statusCode === 400) errorType = 'ValidationError';
      ...
      errorType = 'UnauthorizedError'; // Conta bloqueada
